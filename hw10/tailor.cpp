...
void tailor::randwalk(town & vill)
{
  ...
  //Conditionsals for percent chance and if something is in the way
  if(direction==RAN1)
  {
    where1-=1;
...
//or if there is a phantom.----------------------------------------------------
void tailor::action(town &vill, bully &bul)
{
  //Following conditionals check all around the tailor for actions.
  ...
  else if(!vill.check(where1-DECREMENT, where2))
  {
  ...
  else if(!vill.check(where1, where2+INCREMENT))
  {
  ...
  else if(!vill.check(where1, where2-DECREMENT))
  {
  ...
  //---- end conditions for actions-----

  //Sets the tailor in a location.
  vill.setchr(where1, where2, loca);

  //Finds if the tailor is stepping on phantom pants and kills him if so.
  if(vill.gettile(where1,where2)==PANTS)
  {
    cout << name <<"Has died of a phantom pants attack.\n";
    cout << endl;
    alive = DEAD;
  }
  checkdead();
